...
// Função para salvar imagem com URL completa
...
export async function createCombo(req, res) {
    try {
        ...
        console.log('✅ Combo criado:', combo);
...
export async function updateCombo(req, res) {
    try {
        ...
        if (req.file) {
            ...
            image_url = saveImage(req.file);
            console.log('✅ Nova imagem salva em:', image_url);
        }
        ...
        console.log('✅ Combo atualizado:', combo);
        res.json(combo);
    } catch (error) {
...
export async function getComboItems(req, res) {
    try {
        const items = await Combo.getItems(req.params.id);
        res.json(items);
    } catch (error) {
        console.error('❌ Erro ao listar itens do combo:', error);
        res.status(500).json({ error: error.message });
    }
}